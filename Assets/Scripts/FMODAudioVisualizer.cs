...
public class FMODAudioVisualizer : MonoBehaviour
{
    ...
    [SerializeField] private string eventPath = null;
    ...
    public string EventPath { get => eventPath; set => eventPath = value; }
    ...
    private void SpawnMeterObjects()
    {
        ...
        for (int i = 0; i < freqRanges.Count; i++)
        {
            ...
            if (bandMeters[i].GetComponent<ParamBar>() != null)
            {
                bandMeters[i].GetComponent<ParamBar>()._band = posOffSet;
            }
            posOffSet++;
...
//script modified from Scott Game Sounds
// https://scottgamesounds.com/c-scripts/